/*
 * AIR PURIFIER FIRMWARE — ESP32 + GP2Y1010AU0F
 * ESP-IDF v6.0+ (FreeRTOS) — New I2C master + ADC oneshot drivers
 *
 * Hardware:
 *   - ESP32 DevKit
 *   - GP2Y1010AU0F Sharp Dust Sensor (analog, LED-pulsed)
 *   - 0.96" OLED SSD1306 (I2C)
 *   - 4x TTP223 Touch Sensor Modules
 *   - SG90 Servo Motor (via LEDC PWM)
 *   - Buzzer
 *   - Heater control (optional, kept for parity)
 */
...
typedef struct {
    ...
    volatile bool isSleepMode;
    volatile int  sysMode;
    volatile int  fanSpeedMode;
...
{
    nvs_handle_t h;
    if (nvs_open("airpurifier", NVS_READWRITE, &h) == ESP_OK) {
        float life = 100.0f;
...
static void handle_command(char c)
{
    ...
    case '2':
        if (g_state.isPoweredOn && !g_state.isSleepMode) {
            g_state.sysMode = 0;
    ...
    case '5':
        if (g_state.isPoweredOn) {
            g_state.isSleepMode = !g_state.isSleepMode;
            if (g_state.isSleepMode) {
                g_state.sysMode = 0;
                g_state.fanSpeedMode = 2;
                servo_write(ANG_MID);
                set_last_action("SLEEP MODE");
            } else {
                set_last_action("SLEEP OFF");
            }
            beep();
        }
        break;
...
static void button_task(void *arg)
{
    ...
    const int64_t debounce_us = 120000;
...
static void tcp_server_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "TCP server listening on port 8080");
    ...
    while (1) {
        ...
        ESP_LOGI(TAG, "App client connected");
        while (1) {
            char tx_buf[128];
            const char *mode_str = !g_state.isPoweredOn ? "OFF" :
                                   (g_state.isSleepMode ? "SLEEP" :
                                   (g_state.sysMode ? "AUTO" : "MANUAL"));
            snprintf(tx_buf, sizeof(tx_buf),
                     "aqi %d\npm25 %d\npm10 %d\nfilter %d\ntimer %d\nmode %s\n",
                     g_state.pm25, g_state.pm25, g_state.pm10,
                     (int)g_state.displayedLife, g_state.timerHours, mode_str);
            send(client_sock, tx_buf, strlen(tx_buf), 0);

            int len = recv(client_sock, rx_buf, sizeof(rx_buf) - 1, MSG_DONTWAIT);
            if (len > 0) {
                for (int i = 0; i < len; i++) {
                    if (rx_buf[i] >= '1' && rx_buf[i] <= '5')
                        handle_command(rx_buf[i]);
                }
            } else if (len == 0) {
                ESP_LOGI(TAG, "App client disconnected");
                break;
            } else if (errno != EAGAIN && errno != EWOULDBLOCK) {
                ESP_LOGI(TAG, "App client error");
                break;
            }
...
static void draw_screen(void)
{
    ...
    oled_draw_string(0, 0,
        ...
        g_state.isSleepMode  ? "SLEEP MODE" :
